...
const SendEmail = ({ onClose }) => {
  ...
  const handleSend = async () => {
    if (selectedRecipients.length === 0 || !subject || !body) {
      alert("Please add at least one recipient, a subject, and a message body.");
      return;
    }
    try {
      setIsSending(true);
  ...
  const handleSaveDraft = async () => {
    if (!subject && !body) { onClose(); return; }
    try {
      setIsSavingDraft(true);
      if (selectedRecipients.length > 0) {
        await Promise.all(
          selectedRecipients.map((r) =>
            composeEmail({ recipient_id: r.id, subject: subject || "(No Subject)", body: body || "", is_draft: true, attachment: selectedFile })
          )
        );
      } else {
        await composeEmail({ subject: subject || "(No Subject)", body: body || "", is_draft: true, attachment: selectedFile });
      }
      setDraftSaved(true);
      setTimeout(() => {
        onClose();
          ...
          <div className="flex flex-col flex-1 overflow-y-auto min-h-0">
            ...
            <div className="bg-white flex-1 flex flex-col">
              <div
                ...
                className="flex-1 w-full px-4 py-3 outline-none text-[13px] min-h-[150px] sm:min-h-[200px] overflow-y-auto whitespace-pre-wrap"
                style={{ wordBreak: 'break-word' }}
              />

              {/* Placeholder trick since contentEditable doesn't support the raw placeholder attribute well */}
              {!body && (
                <div className="pointer-events-none absolute text-[13px] text-gray-400 px-4 py-3" style={{ top: bodyRef.current?.offsetTop || 'auto' }}>
                  Body Text
                </div>
              )}
          ...
          <div className="flex items-center justify-between gap-2 px-4 py-3 border-t border-gray-100 bg-white flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={handleSend}
                disabled={isSending}
                className={`text-white px-6 py-2 rounded-lg text-[14px] font-medium transition-colors shadow-sm ${isSending ? "bg-gray-400 cursor-not-allowed" : "bg-[#326C6A] hover:bg-[#2a5957]"
                  }`}
              >
              ...
              <div className="flex items-center gap-3 text-gray-400 ml-1">
                ...
                <div className="relative" ref={emojiRef}>
                  ...
                  {showEmoji && (
                    <div className="absolute bottom-8 left-0 bg-white border border-gray-200 rounded-xl shadow-lg p-2 flex flex-wrap gap-1 w-48 z-50">
            ...
            <div className="flex items-center gap-3 text-gray-400">
              ...
              <div className="relative" ref={moreRef}>
                ...
                {showMoreMenu && (
                  <div className="absolute bottom-8 right-0 bg-white border border-gray-200 rounded-xl shadow-lg py-1 w-40 z-50">
                    <button
                      onClick={() => {
